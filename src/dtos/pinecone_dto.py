...
class LeafletCount(int, Enum):
    count: conint(ge=-1, le=25)


class LeafLength(BaseModel):
    length: conint(ge=-1, le=50) = Field(description="잎길이는 -1에서 50 사이의 값이어야 합니다.")
...
class LeafWidth(BaseModel):
    width: conint(ge=-1, le=30) = Field(description="잎너비는 -1에서 30 사이의 값이어야 합니다.")
...
class QueryPineconeRequestDto(BaseModel):
    ...
    leaflet_count: int = Field(-1, description="소엽갯수는 -1에서 25 사이의 값이어야 합니다.")
    leaf_length: int = Field(-1, description="잎길이는 -1에서 50 사이의 값이어야 합니다.(cm)")
    leaf_tip: LeafTipEnum = Field(LeafTipEnum.unknown, description="잎끝은 미두, 점첨두, 예두, 급첨두, 둔두, 원두, 요두, 평두, 모름 중 하나여야 합니다.")
    leaf_width: int = Field(-1, description="잎너비는 -1에서 30 사이의 값이어야 합니다.(cm)")
